...
export default function AdminPage() {
      ...
      <div className="min-h-screen flex flex-col">
        ...
        <main className="flex-1 flex items-center justify-center px-4">
          <form
            ...
            className="bg-white border border-stone-200 rounded-2xl p-8 w-full max-w-sm shadow-sm"
          >
            ...
            <button
              type="submit"
              className="mt-3 w-full bg-brand-500 text-white py-2.5 rounded-lg text-sm font-medium hover:bg-brand-600 transition-colors"
            >
              Sign In
            </button>
          </form>
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setMessage({ type: "error", text: "Failed to save review. Check S3 config." });
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleImport(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setMessage({
        type: "success",
        text: `Imported ${result.imported} reviews successfully!`,
      });
      fetchReviews();
    } catch {
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-stone-500 text-sm">
              Create, import, and manage your reviews.
            </p>
          </div>
          <button
        ...
        <div className="mt-6 flex gap-1 bg-stone-100 p-1 rounded-xl w-fit">
          {(["create", "import", "manage"] as Tab[]).map((t) => (
            ...
            >
              ...
              {t === "manage" && <Save className="w-4 h-4 inline mr-1" />}
              {t}
            </button>
          ))}
        </div>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setForm({
                      ...form,
                      priceRange: parseInt(e.target.value) as 1 | 2 | 3 | 4,
                    })
                  }
                  className="w-full px-3 py-2 rounded-lg border border-stone-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500"
                >
              ...
              <div className="flex items-end">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    onChange={(e) =>
                      setForm({ ...form, wouldReturn: e.target.checked })
                    }
          ...
          <div className="mt-6 space-y-6">
            <div className="bg-white border border-stone-200 rounded-xl p-6">
              ...
              <p className="mt-1 text-sm text-stone-500">
                Upload your Google Takeout GeoJSON file to bulk-import reviews.
                Categories will be auto-detected from business names.
              </p>
              <div className="mt-4">
                <label className="flex items-center gap-3 cursor-pointer bg-stone-50 border-2 border-dashed border-stone-300 rounded-xl p-8 hover:border-brand-400 transition-colors">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-stone-400">
                      Exported from Google Takeout → Maps (your places) → Reviews
                    </p>
                  </div>
                  <input
                    type="file"
            ...
            <div className="space-y-3">
                ...
                >
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-stone-400">
                      ...
                      {review.source !== "manual" && ` · from ${review.source}`}
                    </p>
                  </div>
                  <button
                    onClick={() => handleDelete(review.id)}
                    className="ml-3 p-2 text-stone-400 hover:text-red-500 transition-colors"
                  >
              ...
              {reviews.length === 0 && (
                <p className="text-center py-8 text-stone-400 text-sm">
                  No reviews yet. Create one or import from Google Takeout.
                </p>
              )}
            </div>
          </div>
